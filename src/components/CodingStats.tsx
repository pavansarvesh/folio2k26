    ...
    <div>
      ...
      <ol className="mt-4 divide-y divide-white/10">
        {languages.slice(0, 5).map((lang, idx) => {
          ...
          return (
            <li key={lang.name} className="flex items-center justify-between gap-6 py-3">
              <div className="flex min-w-0 items-center gap-3">
                <span className="w-5 text-xs tabular-nums text-white/40">{idx + 1}.</span>
                <span className={`truncate text-sm ${isTop ? "font-semibold text-white" : "text-white/80"}`}>
                  {lang.name}
                </span>
              </div>
              <span className="text-sm tabular-nums text-white/60">{percent.toFixed(1)}%</span>
            </li>
          )
        })}
      </ol>
